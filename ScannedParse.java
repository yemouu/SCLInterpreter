...
public class ScanandParse {
    ...
    public static int SCLScanner(ArrayList<String> tokens) throws FileNotFoundException {
        ...
        //For each line, it splits the line into an array of strings using the split method with a space as the delimiter. 
        //It then iterates over the array of strings and adds each non-empty string to an ArrayList called tokens. 
        //The loop also increments a variable called numberOfLines for each line that is read from the file. Finally, the loop returns the total number of lines in the file as an integer value.
        while (fileScan.hasNext()) {
            ...
            // for the length of the current line, we are adding into the parameter tokens, the value of token from the given file
            ...
            //number of lines is incremented and eventually returned to ensure main method is iterating through all lines of the file
            ++lines;
        }
        return lines;
    }
    ...
    public static String tokenType(String token) {
        ...
        //The method initializes an empty string variable called tok_id. It then creates an ArrayList of String objects called identifiers that contains a list of valid identifier names in the SCL language.
        String tok_id = "";
        ArrayList<String> identifiers = new ArrayList<String>(Arrays.asList( "vara", "varb", "varc1","varc2", "varc3", "varc4", "d1", "d2","a", "b", "c"));


        //The method then creates a ListIterator object called identifiersIterator that iterates over the identifiers ArrayList.
        ...
        //The method then creates an ArrayList of String objects called keywords that contains a list of valid keywords in the SCL language.
        ArrayList<String> keywords = new ArrayList<String>(Arrays.asList("import", "description", "symbol", "global", "variables", "define", "of", "type", "unsigned", "integer", "short", "long", "implementations", "function", "is", "begin", "set", "display", "exit", "endfun", "declarations","byte","main"));

        //The method then creates a ListIterator object called keywordsIterator that iterates over the keywords ArrayList.
        //If it finds a token that fits in the keywords arraylist, it will give it the keyword token type
        ListIterator keywordsIterator = keywords.listIterator();
        ...
        //The method then creates an ArrayList of String objects called operators that contains a list of valid operators in the SCL language.
        ArrayList<String> operators = new ArrayList<String>(Arrays.asList("+", "-", "*", "/", "=", "%", "<", ">",
                "<=", ">=", "==", "!=", "(", ")", "++", "--", "!", "&&", "||", "+=", "-=", "*=", "/=", "%="));
               //The method then creates a ListIterator object called operatorIterator that iterates over the operators ArrayList.
                ListIterator operatorIterator = operators.listIterator();
    ...
    //Main method and second half of parser, which is listing the import, description, global declarations, function, and exit tokens sections
    public static void main(String[] args) throws FileNotFoundException {
        ...
        //Once all the inputs have been tokenized and stored in the tokens arraylist, the main method iterates over the tokens arraylist and prints what Statement parses what tokens
